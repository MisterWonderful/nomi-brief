    ...
    <>
      ...
      <div className="space-y-5 sm:space-y-6 max-w-4xl">
        ...
        <div className="flex items-center gap-2">
          <Link href="/articles">
            <Button variant="ghost" size="sm" className="text-zinc-500 hover:text-white text-xs sm:text-sm -ml-2">
              <ArrowLeft className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" />
              Articles
            </Button>
          </Link>
          <span className="text-zinc-800">·</span>
          <span className="text-zinc-600 text-xs">{article.category}</span>
        </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          {/* Category */}
          <span className="inline-block px-2.5 py-1 text-[10px] sm:text-xs font-semibold bg-violet-500/20 text-violet-400 border border-violet-500/30 rounded-full">
            {article.category}
          </span>
          ...
          {/* Subtitle */}
          {article.subtitle && (
            <p className="text-sm sm:text-base md:text-lg text-zinc-400 leading-relaxed font-light">
              {article.subtitle}
            </p>
          )}

          {/* Meta — stacked on mobile, row on desktop */}
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 py-3 border-y border-zinc-800/60">
            {/* Author */}
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
                {article.authorName.charAt(0)}
              </div>
              <div>
                <p className="text-white text-xs sm:text-sm font-medium leading-none">{article.authorName}</p>
                <p className="text-zinc-600 text-[10px] sm:text-xs mt-0.5">{timeAgo}</p>
              </div>
            </div>
          ...
          {article.tags.length > 0 && (
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {article.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 text-[10px] sm:text-xs font-medium bg-zinc-800/60 text-zinc-400 rounded-lg border border-zinc-700/30"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-4 border-t border-zinc-800/60">
          ...
          <Link href="/articles">
            <Button variant="ghost" size="sm" className="text-zinc-600 hover:text-white text-xs sm:text-sm">
              ← Back
            </Button>
          </Link>
        </div>
        ...
        {/* Related */}
        <Link href="/articles">
          <div className="rounded-xl p-5 glass hover:glow-hover transition-all cursor-pointer">
      ...
      <style>{`
        .youtube-embed-wrapper {
          ...
          margin: 1.5rem 0; border: 1px solid rgba(255,255,255,0.06);
          background: #0a0a0a;
        }
